...
<template>
  <div class="flex flex-col gap-4 flex-1">
    <!-- 顶部工况参数栏：循环圈数与单圈/总计耗时概览 (用户自定义极简结构) -->
    <div class="flex flex-wrap items-center justify-between gap-2.5 p-2.5 px-3 bg-muted/25 rounded-xl border border-border/60">
      <!-- 循环圈数配置 -->
      ...
      <div class="flex items-center gap-3 text-xs text-muted-foreground font-mono">
        ...
        <div v-if="cycle.totalLoops.value > 0" class="flex items-center gap-1">
          <span>总计:</span>
        ...
        <Button
          ...
          size="sm"
          variant="outline"
          class="h-7 px-2.5 text-xs text-muted-foreground hover:text-foreground hover:bg-muted active:scale-[0.98] transition-all"
          title="恢复初始的正反转标准步骤"
    ...
    <!-- 实时执行状态看板 (仅在非待命状态或正在运行时醒目呈现) -->
    <div
      v-if="!cycle.isIdle.value || cycle.status.value === 'completed'"
      class="p-3 bg-muted/40 rounded-xl border border-border/80 flex flex-col gap-2.5 transition-all"
    >
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <Badge :variant="statusBadgeVariant" class="text-xs px-2 py-0.5">
            <span
              v-if="cycle.isRunning.value"
              class="inline-block size-1.5 rounded-full bg-current mr-1.5 animate-pulse"
            />
            {{ statusText }}
          </Badge>
          <span class="text-xs text-muted-foreground font-mono">
            第 {{ cycle.currentLoop.value }} / {{ cycle.totalLoops.value === 0 ? '∞' : cycle.totalLoops.value }} 圈
          </span>
        </div>
      ...
      <!-- 当前步骤详情与倒计时 -->
      <div v-if="cycle.currentStep.value" class="flex flex-col gap-1.5 pt-1">
        <div class="flex items-center justify-between text-xs">
          <div class="flex items-center gap-2">
            <span class="font-semibold text-foreground">
              [步骤 {{ cycle.currentStepIndex.value + 1 }}/{{ cycle.steps.value.length }}] {{ cycle.currentStep.value.name }}
            </span>
        ...
        <!-- 总体进度条 (仅在设定了圈数时显示) -->
        <div v-if="cycle.totalLoops.value > 0" class="flex flex-col gap-1 mt-1">
          <div class="flex items-center justify-between text-[11px] text-muted-foreground">
            <span>总进度 ({{ cycle.currentLoop.value }}/{{ cycle.totalLoops.value }} 圈)</span>
          ...
          >
            ...
            <div class="relative w-25 sm:w-28 shrink-0">
              <Input
                ...
                min="-12000"
                max="12000"
                class="h-7 px-2 pr-8 text-xs font-mono font-bold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                :disabled="cycle.isRunning.value || cycle.isPaused.value || step.gear === 3"
              />
              <span class="absolute inset-y-0 right-1.5 flex items-center pointer-events-none text-[10px] text-muted-foreground font-mono">
                RPM
              </span>
            </div>
            ...
            <div class="relative w-18 sm:w-20 shrink-0">
              <Input
                ...
                min="1"
                max="3600"
                class="h-7 px-2 pr-5 text-xs font-mono font-bold text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                :disabled="cycle.isRunning.value || cycle.isPaused.value"
              />
              <span class="absolute inset-y-0 right-1.5 flex items-center pointer-events-none text-[10px] text-muted-foreground font-mono">
                s
    ...
    <div class="flex items-center gap-3 pt-1">
      ...
      <template v-if="cycle.isIdle.value">
        <Button
          size="sm"
          variant="default"
          class="h-10 w-full font-semibold text-sm transition-all"
      ...
      <template v-else-if="cycle.isRunning.value">
        <Button
          size="sm"
          variant="outline"
          class="h-10 flex-1 font-semibold text-sm border-warning/50 text-warning hover:bg-warning/10 transition-all"
        ...
        <Button
          size="sm"
          variant="destructive"
          class="h-10 flex-1 font-semibold text-sm transition-all"
