...
const dcPowerKw = computed(() => {
  const p =
    (mcu1.value.mcu_dc_main_wire_volt * mcu1.value.mcu_dc_main_wire_curr) /
    1000;
  return p.toFixed(2);
});
  ...
  <Card class="border-border shadow-xs h-full flex flex-col">
    <CardHeader class="pb-3 border-b bg-muted/20">
      ...
      >
        <div class="flex flex-wrap items-center gap-2">
          <CardTitle class="text-base font-semibold"> MCU 实时反馈 </CardTitle>
          <Badge
            :variant="
              hasFreshMcu2Feedback && mcu2.mcu_lv_sts === 1
                ? 'success'
                : 'secondary'
            "
          ...
          <Badge
            :variant="
              hasFreshMcu2Feedback && mcu2.mcu_en_sts === 1
                ? 'success'
                : 'secondary'
            "
            class="font-mono text-xs px-1.5 py-0"
    ...
    <CardContent class="p-4 sm:p-5 flex-1 flex flex-col justify-between gap-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <!-- 驱动电机温度 -->
        <div
          class="p-3.5 rounded-xl border border-border/60 bg-muted/25 flex flex-col justify-between gap-1.5"
        >
          <div
              ...
              :class="
                cn({
                  'bg-success shadow-xs shadow-success/50 animate-pulse':
                    tbox.st_mtr === 1,
                  'bg-info shadow-xs shadow-info/50 animate-pulse':
                    tbox.st_mtr === 2,
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
        <div
          class="p-3.5 rounded-xl border border-border/60 bg-muted/25 flex flex-col justify-between gap-1"
        >
        ...
        >
          ...
          >
            {{
              mcu2.mcu_active_dischrg_sts === 0
                ? "放电中"
                : mcu2.mcu_active_dischrg_sts === 1
                  ? "放电完成"
                  : mcu2.mcu_active_dischrg_sts === 2
                    ? "放电失败"
                    : "未触发"
            }}
          </div>
